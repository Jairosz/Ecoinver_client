...
export class DashboardComponent implements OnInit {
  ...
  constructor(private comercialServicio: ComercialServiceService, private generoServicio: GenderService, private planingComercial: ComercialPlanningService, private plannigSemanas: ComercialPlanningDetailsService, private cultiveServiceProduction: CultiveProductionService, private cultivoServicio: CultivoService
  ) { }
  ...
  ngOnInit(): void {
    ...
    this.cultiveServiceProduction.getAllCultiveProductions().subscribe(
      (data) => {
        ...
        console.log(this.cultiveProductions);
      },
      (error) => {
  ...
  initializeCharts() {
    ...
    if (generosSeleccionados[0]) {
      ...
      for (let i = 0; i < this.cultive.length; i++) {
        if (this.cultiveProductions.filter(item => item.cultiveId == this.cultive[i].id) && this.selectedProductions.filter(item => item.cultiveId != this.cultive[i].id)) {
  ...
  exportToPdf(): void {
    try {
      ...
      // Primero mostramos las tablas y luego la gráfica al final
  
      // --- TABLA 1: NECESIDADES COMERCIALES ---
      const colorPrincipal = [79, 70, 229]; // Color principal (indigo) para ambas tablas
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(12);
      doc.text('NECESIDADES COMERCIALES', margin + 5, yPos + 5.5);
  
      // Subtítulo
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(colorPrincipal[0], colorPrincipal[1], colorPrincipal[2]);
      doc.text('Registros Filtrados', margin + 75, yPos + 5.5);
      ...
      if (self.filteredComNeeds && self.filteredComNeeds.length > 0) {
        ...
        const colWidths = [
          Math.round(totalWidth * 0.23), // 23% para Género
          Math.round(totalWidth * 0.23), // 23% para Código Cliente
          Math.round(totalWidth * 0.14), // 14% para KGs
          Math.round(totalWidth * 0.40)  // 40% para Nombre Comercial
        ];
  
        const rowHeight = 10; // Altura de fila aumentada para mejor legibilidad
        ...
        yPos += rowHeight;
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(8); // Tamaño reducido para mejor ajuste
  
        data.forEach((row, rowIndex) => {
          // Alternar color de fondo para las filas
          if (rowIndex % 2 === 0) {
            doc.setFillColor(240, 242, 251); // Azul muy claro
            doc.rect(margin, yPos, contentWidth, rowHeight, 'F');
          }
          ...
          // Procesar todas las columnas - sin truncar texto
        ...
        doc.text('No hay datos disponibles de necesidades comerciales con los filtros actuales.', margin + 5, yPos + 10);
        yPos += 20;
      }
      ...
      // --- TABLA 2: PRODUCCIÓN DE CULTIVOS ---
      // Comprobar si necesitamos una nueva página para la segunda tabla
      ...
      doc.setFillColor(colorPrincipal[0], colorPrincipal[1], colorPrincipal[2]); // Mismo color que la primera tabla
      doc.rect(margin, yPos, 70, 8, 'F');
      doc.setTextColor(255, 255, 255);
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(12);
      ...
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(colorPrincipal[0], colorPrincipal[1], colorPrincipal[2]);
      doc.text('Producciones Seleccionadas', margin + 75, yPos + 5.5);
      ...
      if (self.selectedProductions && self.selectedProductions.length > 0) {
        ...
        produccionData.forEach((row, rowIndex) => {
          // Alternar color de fondo para las filas
          if (rowIndex % 2 === 0) {
            doc.setFillColor(240, 242, 251); // Mismo color que la primera tabla
            doc.rect(margin, yPos, contentWidth, rowHeight, 'F');
          }
      ...
      // --- SECCIÓN 3: GRÁFICA (AHORA AL FINAL DEL PDF) ---
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(14);
      doc.text('VISUALIZACIÓN GRÁFICA - ' + this.vistaSeleccionada.toUpperCase(), pageWidth / 2, yPos + 6.5, { align: 'center' });
      ...
      // Capturar e insertar la gráfica
